...
bool BFSSolver::step() {
    ...
    // Note: In standard BFS, we pop and process neighbors.
    // For step-by-step visuals, usually popping one node = one step.
    ...
    for (const size_t V_NODE : (*graph_)[U_NODE]) {
        if (!visited_[V_NODE]) {
            ...
            // Optimization: Detect target early if desired, but 
            // standard BFS checks for target when popping.
            // We'll stick to queue processing order for strict visual BFS.
        }
    }
