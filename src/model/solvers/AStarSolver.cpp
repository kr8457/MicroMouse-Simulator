 ...
 * @brief Implementation of the A* pathfinding algorithm for maze solving.
 */
...
void AStarSolver::initialize(const std::vector<Cell>& /*grid*/, 
    ...
    if (total_nodes_ > 0) {
        ...
        auto heuristic = [&](size_t n) -> double {
            auto [r1, c1] = MazeUtils::get_2d_coords(n, cols_);
            auto [r2, c2] = MazeUtils::get_2d_coords(end_node_, cols_);
